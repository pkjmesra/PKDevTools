...
class RepairDB:
    ...
    def rebuild_all_indexes(self) -> bool:
        ...
        try:
            ...
            conn.execute("PRAGMA foreign_keys=OFF")  # Disable FK checks during rebuild
            ...
            with conn:
                for name, create_sql in indexes:
                    try:
                        ...
                        create_sql = create_sql.replace("CREATE INDEX", "CREATE INDEX IF NOT EXISTS")
                        create_sql = create_sql.replace("CREATE UNIQUE INDEX", "CREATE UNIQUE INDEX IF NOT EXISTS")
    ...
    def emergency_repair(self):
        subprocess.run(["sqlite3", self.local_db_path, ".recover | sqlite3 repaired.db"])
