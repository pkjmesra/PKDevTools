...
from PKDevTools.classes.DBManager import DBManager, PKScannerJob, PKUser, PKUserModel
...
class TestDBManager:
    def test_connection(self, db_manager):
        ...
        libsql.connect.assert_called_once_with(
            database="test.db", auth_token="test_token"
        )
    ...
    def test_get_scanner_jobs_with_active_users(
        self, db_manager, sample_scanner_job):
        """Test retrieving scanner jobs with active users."""
        manager, _, mock_cursor = db_manager
        mock_cursor.fetchall.return_value = [("SCAN1", "123;456")]
